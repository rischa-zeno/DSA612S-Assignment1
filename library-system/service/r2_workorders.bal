...
resource function put [string workOrderId]/status(string status) returns
WorkOrder|http:NotFound|http:BadRequest {
...
return <http:BadRequest>{
body: {message: "Cannot transistion from: " + existing.status + "to" + status, errorCode: "INVALID_STATUS_TRANSITION", timestamp: nowTimestamp()}
};
}
...
resource function post [string workOrderId]/subtasks(SubTask newTask) returns
WorkOrder|http:NotFound|http:BadRequest {
WorkOrder? existing = workOrderStore[workOrderId];
if existing is () {
return <http:NotFound>{
